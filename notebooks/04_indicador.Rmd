...
Se observa un desplazamiento de la curva hacia abajo en el corto, mediano y a inicios del largo plazo. Se observa una mayor curvatura a inicios del largo plazo entre la última semana de abril y finales de junio aproximadamente. La tendencia se observan curvas ascendentes.
...
plotdist(base_temp2$IR_S, histo = TRUE, demp = TRUE)

descdist(base_temp2$IR_S, boot = 1000)
...
plotdist(as.numeric(datos_long[10,8:3608]), histo = TRUE, demp = TRUE)

descdist(as.numeric(datos_long[10,8:3608]), boot = 1000)

```
`
```{r}

pre_gamma = fitdist(as.numeric(datos_long[10,8:3608]), "lnorm")
denscomp(pre_gamma)
qqcomp(pre_gamma)
cdfcomp(pre_gamma)
ppcomp(pre_gamma)
...
componentes2 <- componentes %>% 
  dplyr::select(-c(semana_rango, PC1, PC2, PC3)) %>% 
  mutate(IR_R = (IR-min(IR))/(max(IR)-min(IR)))

fb <- fitdist(componentes2$IR_R, "weibull")
fg <- fitdist(componentes2$IR_R, "gamma")
fln <- fitdist(componentes2$IR_R, "lnorm")
par(mfrow = c(2, 2))
plot.legend <- c("Beta", "lognormal", "gamma")
denscomp(list(fb, fln, fg), legendtext = plot.legend)
qqcomp(list(fb, fln, fg), legendtext = plot.legend)
