...
serie<-ts(componentes$IR_S, start = 1)
...
La tendencia es positiva, las curvas a corto plazo tienen menores rendimientos que los de las curvas a largo plazo que son mayores. En cuanto a la curvatura vemos que a finales del corto plazo se empieza a formar.
...
caso 2: disminución en el indicador pero con cierta variabilidad


Se observa un desplazamiento de la curva hacia abajo en el corto, mediano y a inicios del largo plazo. Se observa una mayor curvatura a inicios del largo plazo entre la última semana de abril y finales de junio aproximadamente. La tendencia es positiva.


```{r}

datos %>%
  filter(fecha>="2020-04-28" ) %>% 
  arrange(fecha) %>% 
  mutate(fecha=as.factor(fecha)) %>% 
  ggplot(aes(x = plazo_en_anos, y = tasa_percent, colour = fecha)) +
  geom_line() +
...
La tendencia es positiva, las curvas a corto plazo tienen menores rendimientos que los de las curvas a largo plazo que son mayores. La curvatura se observa principalmente en el mediano plazo.
